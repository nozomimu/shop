class UsersController < ApplicationController
  ...
  def show
    ...
    # 履歴表示
    @orders_for_user = Order.where(user_id:@user.id)
    @orders = Order.where(user_id: @user.id).order("id desc") #ログインユーザーの履歴を一覧化
    #@history_ids = @orders.pluck(:detail_id).uniq #@historiesからhistory_address_idの一覧を取得して配列にする
  ...
  def destroy
    ...
    redirect_to root_path#User#index から rootへ変更
  end
end
